...
function LoginScreen({onLogin}){
  ...
  const selUser = USERS_DEF.find(u=>u.id===uid2);

  const handleLogin=()=>{
    if(!uid2){setErr("Vui lòng chọn tài khoản!");return;}
    const u=USERS_DEF.find(u=>u.id===uid2&&u.pw===pw);
    if(!u){setErr("Mật khẩu không đúng!");return;}
    setErr("");onLogin(u,USERS_DEF);
  };
        ...
        <div style={{marginBottom:16}}>
          ...
          <select value={uid2} onChange={e=>{setUid2(e.target.value);setErr("");setPw("");}} style={si}>
            ...
            <optgroup label="🏭 Nhà máy THCK" style={{background:"#1e3a5f"}}>
              {USERS_DEF.filter(u=>u.role==="thck").map(u=>(
            ...
            <optgroup label="🔧 Xưởng Hàn" style={{background:"#1e3a5f"}}>
              {USERS_DEF.filter(u=>u.role==="xuonghan").map(u=>(
                <option key={u.id} value={u.id} style={{background:"#1e3a5f"}}>{u.avatar} {u.ten} ({u.id})</option>
              ))}
            </optgroup>
          </select>
...
function UsersPanel({currentUser, users, setUsers}){
  ...
  const save=()=>{
    ...
    if(editing){
      setUsers(l=>l.map(u=>u.id===editing?{...u,...form}:u));fl("✓ Đã cập nhật");
    } else {
      if(users.find(u=>u.id===form.id)){fl("⚠️ ID đã tồn tại!");return;}
      setUsers(l=>[...l,{...form}]);fl("✓ Đã thêm tài khoản");
    }
  ...
  const del=id=>{
    ...
    setUsers(l=>l.filter(u=>u.id!==id));fl("✓ Đã xóa");
  };
...
export default function App(){
  ...
  const save=()=>{
    ...
    setBomDB(s=>{
      const old=s[pid]||[];
      if(edit)return{...s,[pid]:old.map(v=>v.ma===cur.ma?{...v,...cur}:v)};
      const ns=old.length?Math.max(...old.map(v=>v.stt))+1:1;
      return{...s,[pid]:[...old,{id:uid(),pid,stt:ns,...cur}]};
    });
  ...
  const doIO=()=>{
    ...
    addLS(pid,{pid,ma:cur.ma,ten:cur.ten,loai,sl:modal==="nhap"?sl:-sl,gc:gcXT});
  ...
  const mkProj=()=>{
    ...
    // Commit tất cả state cùng lúc trước khi switch
    setProjs(ps=>[...ps,p]);
    setBomDB(s=>({...s,[id]:bomRows}));
    setNewP(false);
    setNPF({ten:"",moTa:"",mau:"#7c3aed",icon:"📦",so_xe:1,bom:"trong"});
    // Switch sau khi state đã set
    setTimeout(()=>sw(id),0);
  ...
  const editSoXe=()=>{
    ...
    if(v&&!isNaN(v)&&Number(v)>0)setProjs(ps=>ps.map(p=>p.id===pid?{...p,so_xe:Math.round(Number(v))}:p));
  };
  ...
  const doImport=()=>{
    ...
    setBomDB(s=>{
      const old=s[pid]||[];
      if(importMode==="thay") return{...s,[pid]:rows};
      // Thêm vào, bỏ qua mã đã có
      ...
      return{...s,[pid]:[...old,...news.map((v,i)=>({...v,stt:maxStt+i+1}))]};
    });
    setShowImport(false);
  ...
  const guiDon=()=>{
    ...
    ct.forEach(c=>addLS(pid,{pid,ma:c.ma,ten:c.ten,loai:"Xuất kho",sl:-c.sl,gc:`Đơn ${sp}`}));
  ...
  const submitPh=()=>{
    ...
    setPhDB(s=>({...s,[pid]:[ph,...(s[pid]||[])]}));
    phIt.forEach(it=>addLS(pid,{pid,ma:it.ma,ten:it.ten,loai:"Xuất kho",sl:-it.sl,gc:`Phiếu ${phF.sp}`}));
    setShowPh(false);flash(`✓ Tạo phiếu ${phF.sp}`);
  };
  const xacNhan=id=>setPhDB(s=>({...s,[pid]:(s[pid]||[]).map(p=>p.id===id?{...p,tt:"Đã xác nhận"}:p)}));
  ...
  const duyetAll=phid=>{
    ...
    setPhDB(s=>({...s,[pid]:(s[pid]||[]).map(p=>p.id===phid?{...p,tt:"Đã xác nhận",ct:(p.ct||[]).map(c=>({...c,ok:true}))}:p)}));
    ...
    <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif",background:"#f0f4f8",minHeight:"100vh"}}>
      ...
      <div style={{background:isTHCK?"linear-gradient(135deg,#1e3a5f,#1d4ed8)":"linear-gradient(135deg,#431407,#b45309)",color:"#fff",padding:"12px 18px"}}>
        ...
        <div style={{marginTop:10,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
          ...
          {isTHCK&&<button onClick={()=>setNewP(true)} style={{...btn,background:"rgba(255,255,255,0.2)",color:"#fff",border:"1px solid rgba(255,255,255,0.3)"}}>+ Thêm dự án</button>}
          {isTHCK&&projs.length>1&&<button onClick={()=>delProj(pid)} style={{...btn,background:"rgba(220,38,38,0.35)",color:"#fca5a5",fontSize:11}}>🗑 Xóa</button>}
      ...
      <div style={{padding:"14px 18px"}}>
        ...
        {tab==="ds"&&(
          <div>
            <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap",alignItems:"center"}}>
              ...
              <div style={{marginLeft:"auto",display:"flex",gap:8}}>
                {isXH&&<button onClick={()=>setShowImport(true)} style={{...btn,background:"#f3f4f6",color:"#374151",padding:"7px 14px",fontSize:13,border:"1px solid #e5e7eb"}}>📥 Import BOM</button>}
                ...
                {isTHCK&&<span style={{fontSize:12,color:"#9ca3af",padding:"7px 0",fontStyle:"italic"}}>👁 Chế độ xem</span>}
              </div>
            </div>
        ...
        {tab==="users"&&isXH&&(
          <UsersPanel currentUser={user} users={users} setUsers={setUsers}/>
        )}
